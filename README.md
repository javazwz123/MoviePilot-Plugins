...
插件采用 `plugins/nodeseeksign/` 二级目录与 `package.json + v2=true` 的单实现 V2 兼容结构，并继续使用 `nodeseeksign` ID、`nodeseeksign_` 配置前缀、`sign_history` 和 `last_sign_date` 数据键，可直接原位升级。为避免同 ID 来源冲突，请在 MoviePilot 中只保留一个包含该插件的第三方仓库来源。
