...
插件继续使用 `nodeseeksign` ID、`nodeseeksign_` 配置前缀、`sign_history` 和 `last_sign_date` 数据键，可直接原位升级。为避免同 ID 来源冲突，请在 MoviePilot 中只保留一个包含该插件的第三方仓库来源。

## 开发验证

```bash
python3 -m compileall -q plugins.v2
python3 -m unittest discover -s tests -v
python3 scripts/check_plugin_versions.py package.json package.v2.json
```
